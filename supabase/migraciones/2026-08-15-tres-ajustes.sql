...
--
-- CORRECCIÓN post-revisión: el diseño original del brief usaba el errcode
-- por defecto P0001 tanto para la señal de reversión como para CUALQUIER
-- `raise exception` de negocio sin errcode explícito (p. ej. el rechazo de
-- importar_planilla cuando la empresa no está activa, que también usa RAISE
-- EXCEPTION sin USING ERRCODE ⇒ P0001 por defecto). Como el `exception when
-- sqlstate 'P0001'` atrapaba TODO lo que llegara con ese código, el rechazo
-- de negocio quedaba envuelto también, y `sqlerrm::jsonb` fallaba (el mensaje
-- del rechazo de negocio no es jsonb válido) devolviendo un error confuso
-- ("invalid input syntax for type json") en vez del mensaje de negocio claro.
-- Reproducible con previsualizar_importacion('bremco', '[]'::jsonb) (bremco
-- está retirada). Fix: la señal de reversión usa un errcode CUSTOM exclusivo
-- ('PV999', no usado por Postgres ni por ninguna excepción de negocio del
-- proyecto) tanto al lanzarla como al atraparla, así que cualquier otra
-- excepción (negocio o de motor) se propaga sin tocar.
...
begin
  ...
  -- CORRECCIÓN post-revisión: un lote PDF puede ser PARCIAL (solo boletas
  -- corregidas, no todo el personal del periodo) — a diferencia de
  -- publicar_lote, que siempre genera un documento por cada vínculo vigente
  -- de la empresa. Marcar 'reemplazado' TODOS los documentos de versiones
  -- previas del mismo lote (patrón heredado de publicar_lote) le quitaría su
