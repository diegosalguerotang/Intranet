...
-- inventario operativo. Spec: Importacion_Activos.docx + plan
-- docs/superpowers/plans/2026-08-17-importacion-activos.md. Idempotente.
--
-- La identidad del activo sigue siendo el código (PK global). El caso "mismo
-- código en otra empresa" se detecta por empresa_id distinto y BLOQUEA la
-- importación completa (un traslado es otra operación).

-- 1 · Columnas nuevas: lo que el archivo trae y el modelo no tenía.
alter table activos add column if not exists tipo text;                    -- LAPTOP/PC/IMPRESORA/… (detalle del archivo)
alter table activos add column if not exists area text;                    -- separadora de área del inventario
alter table activos add column if not exists asignado_sin_confirmar text;  -- texto de la columna USUARIO: NO vincula al maestro
alter table activos add column if not exists usuario_anterior text;        -- historial textual del archivo
alter table activos add column if not exists observaciones text;

-- 2 · El archivo real trae marca/modelo/serie vacíos o basura (procesadores en
--     la columna de serie): dejan de ser obligatorios y la serie deja de ser
--     única por categoría (la unicidad real la vigila la advertencia de serie
--     repetida del parser, solo sobre valores con forma de serie).
alter table activos alter column marca drop not null;
alter table activos alter column modelo drop not null;
alter table activos alter column serie drop not null;
alter table activos drop constraint if exists activos_categoria_serie_key;

-- 3 · Regla compartida de sobrescritura: un vacío no borra y un prefijo más
--     corto no degrada (misma regla que la importación de personal).
create or replace function fn_valor_importado(p_nuevo text, p_actual text)
...
-- 4 · Importación transaccional: o entra todo o no entra nada. Los bloqueos
--     (duplicado en archivo, código de otra empresa, empresa no activa) son
--     excepciones: revierten la llamada completa.
...
begin
  ...
  -- Duplicado dentro del archivo: bloqueante (la pantalla ya lo listó con
  -- filas y usuarios; esto es la defensa del lado del servidor).
  ...
  for a in select * from jsonb_array_elements(p_activos) loop
    ...
    if v_otra is null then
      ...
      -- Actualización: solo los campos del archivo; jamás estado físico,
      -- valor, compra ni asignaciones. Vacío no borra; prefijo no degrada.
      ...
      -- Diff campo por campo: es lo que la vista previa muestra a quien carga.
      v_cambios := '{}'::jsonb;
  ...
  -- Fila resumen de auditoría (mismo criterio que importar_planilla): la razón
  -- social CONFIRMADA, el archivo, el total y quién confirmó.
...
-- 5 · Vista previa sin rastro: misma lógica, revertida con el errcode custom
--     PV999 (mismo patrón verificado de previsualizar_importacion).
...
-- 6 · v_activos expone los campos nuevos (se agregan AL FINAL: create or
--     replace view exige conservar las columnas existentes en su orden).
