-- supabase/migraciones/2026-09-18-fase3b-datos-bancarios.sql
...
--
-- UNA transacción. Reversión: supabase/respaldos/2026-09-18-fase3b-reversion.sql
...
begin
  ...
  if n <> 6 then raise exception 'fase3b: personas tiene % de las 6 columnas bancarias (¿fase 3b aplicada?)', n; end if;
...
-- 1 · Respaldo de lo que cambia (para la reversión exacta, sin depender del repo).
...
begin
  ...
  select count(*) into n from unnest(array['fn_ultimos4(text)', 'fn_guardar_datos_bancarios(text, text, text, text, text, boolean, text)', 'fn_auditar_datos_bancarios()']) f
