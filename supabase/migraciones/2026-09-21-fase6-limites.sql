-- supabase/migraciones/2026-09-21-fase6-limites.sql
...
-- UNA transacción. Reversión: supabase/respaldos/2026-09-21-fase6-reversion.sql.
...
insert into interno.respaldo_fase6
  select 'fn:' || f, pg_get_functiondef(f::regprocedure) from unnest(array['public.fn_nivel_modulo(text)', 'public.registrar_ingreso(text, text, text)', 'public.portal_registrar_ingreso(text, text, text)', 'public.verificar_bloqueo(text)', 'public.portal_verificar_bloqueo(text)', 'public.guardar_politica(integer, integer, boolean, boolean, integer, integer, text, integer, integer, integer, text)']) as f;
insert into interno.respaldo_fase6
  select 'acl:' || f, coalesce(p.proacl::text, '') from unnest(array['public.fn_nivel_modulo(text)', 'public.registrar_ingreso(text, text, text)', 'public.portal_registrar_ingreso(text, text, text)', 'public.verificar_bloqueo(text)', 'public.portal_verificar_bloqueo(text)', 'public.guardar_politica(integer, integer, boolean, boolean, integer, integer, text, integer, integer, integer, text)']) as f join pg_proc p on p.oid = f::regprocedure;
insert into interno.respaldo_fase6
...
begin
  ...
  if (select clave_longitud_min_backoffice from interno.politica_acceso where id = 1) < 10 then raise exception 'fase6: la política sigue por debajo de 10'; end if;
